...
def calculate_coefficient(mines_count: int, opened_cells_count: int) -> float:
    ...
    cells_count = MinesKeyboards.quad_len**2
...
async def handle_start_mines_callback(callback: CallbackQuery, callback_data: MinesCreationCallback):
    ...
    await callback.message.edit_caption(caption=MinesMessages.get_game_started(), reply_markup=markup)
...
async def handle_mine_callback(callback: CallbackQuery, callback_data: MinesCallback):
    ...
    await callback.message.edit_caption(caption=MinesMessages.get_player_loose(), reply_markup=markup)
...
async def handle_take_winning_callback(callback: CallbackQuery):
    ...
    win_amount = game_data.game.bet * win_coefficient
    win_amount = await winnings.accrue_winnings(
    ...
    await callback.message.edit_caption(caption=MinesMessages.get_player_won(win_amount=win_amount), reply_markup=markup)
